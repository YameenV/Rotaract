import "../../css/portfolio-landing.css";
import ashu from "../../imgs/god/ashutosh.jpeg";
import astro from "../../imgs/astro.png";
import {  ButtonGroup, InputGroup, FormControl } from "react-bootstrap";
// import navbar from "../"

const Landing = () => {
  return (
    <div className="main-11">
      
      <div className="landing-header">
        {/* <img  className="landing-bg-astro" src={astro}></img> */}
        <h1> Rotracter 2021-2022</h1>
        <div className="member-landing_search">
          <InputGroup>
            <FormControl
              className="memberlanding_inputfield"
              placeholder="Search Rotracter"
              aria-label="Username"
            />
          </InputGroup>
        </div>
      </div>
      <div className="landing-containers">
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh Upadhyay</div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh Upadhyay </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh Upadhayay </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh Upadhayay</div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Yameen Vinchu</div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Harsh Patel</div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh Upadhayay</div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh Upadhayay</div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh Upadhayay</div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh Upadhayay</div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh Upadhayay</div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh Upadhayay</div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh Upadhayay</div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh Upadhayay</div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh Upadhayay</div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh Upadhayay</div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh Upadhayay</div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh Upadhayay</div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh Upadhayay</div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh Upadhayay</div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Yameen Vinchu</div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh Upadhayay</div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
        <div className="member-div-1">
          <div>
            <img src={ashu} className="member-div-1-image"></img>
          </div>
          <div className="member-div-1-name">Rtr. Ashutosh </div>
          <div className="member-div-1-status">President</div>
        </div>
      </div>
    </div>
  );
};

export default Landing;
